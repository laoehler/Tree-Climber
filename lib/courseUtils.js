...
const buildRankedSchedules = (results) => {
  ...
  // If no results have matches, return empty array
  ...
  const dfs = (index, currentSchedule) => {
    ...
    // Try to find a course from this selection that doesn't conflict
    ...
    // If no compatible course was found, skip this selection and continue
    if (!foundCompatible) {
      dfs(index + 1, currentSchedule);
    }
  };
